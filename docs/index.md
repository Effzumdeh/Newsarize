...
## ✨ Key Features
* **Material Design 3 & Edge-to-Edge Experience:** The app natively draws underneath system navigation bars, utilizing bottom sheets, `ElevatedCards`, swipe-to-dismiss gestures, and modern typography tokens for an immersive experience.
...
* **Smart UI Queuing:** A background worker silently picks off the newest articles you are currently looking at and summarizes them in milliseconds while keeping your UI utterly butter-smooth (with crash resilient infinite-loop guards!). You can also filter articles by "Alle", "Ungelesen", or "Gelesen".
...
* **Universal XML Engine:** Hybrid Atom & RSS 2.0 unmarshaller reliably pulls feeds from almost any blog regardless of formatting complexities (`<item>` vs `<entry>`). Automatically extracts high-quality article thumbnails and renders them efficiently utilizing **Coil**.
